<?php
// Buffer de saída: protege o header('Location') abaixo mesmo se algum espaço/caractere
// acabar vazando antes do <?php (ex: BOM salvo por um editor de texto no Windows).
ob_start();
...
if ($dbOk) {
    ...
    try {
        ...
        // Garante a tabela mesmo se o init.sql não tiver rodado (ex: volume já existia antes do script)
        mysqli_query($conn, "CREATE TABLE IF NOT EXISTS mensagens (
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty(trim($_POST['texto'] ?? ''))) {
            ...
            // Post/Redirect/Get: evita reenvio do formulário (e duplicação de registros) ao dar F5
            $redirectUrl = ($_SERVER['PHP_SELF'] ?? 'index.php') . '?gravado=1';
            ob_end_clean();
            ...
            // Fallback: se por algum motivo os headers já foram enviados, redireciona via HTML/JS
            echo '<!DOCTYPE html><html><head><meta charset="UTF-8">'
               . '<meta http-equiv="refresh" content="0;url=' . htmlspecialchars($redirectUrl) . '">'
               . '</head><body><script>location.replace(' . json_encode($redirectUrl) . ');</script>'
        ...
        $formSuccess = null;
        $formError = 'Erro ao acessar a tabela "mensagens": ' . $e->getMessage();
    }
